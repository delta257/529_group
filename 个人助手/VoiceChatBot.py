...
from emotion_evaluation import EmotionEvaluator
...
7. 在回答问题时，你应该：
   ...
   - 如果搜索工具不可用，直接使用你的知识回答"""
        ...
        # 初始化视频队列，用于管理录制的视频
        self.video_queue = deque(maxlen=5)  # 最多存储 5 个视频文件路径
    ...
    def toggle_camera(self):
        """Toggle camera state and start video recording if camera is turned on."""
        with self.camera_lock:
            if not self.is_camera_active:
                try:
                    ...
                    self.video_recording_thread = threading.Thread(target=self._record_video)
                ...
                if self.video_recording_thread and self.video_recording_thread.is_alive():
                    self.video_recording_thread.join(timeout=5)
                    if self.video_recording_thread.is_alive():
                        print("Recording thread did not stop within 5 seconds.")
                self.camera.release()
                self.camera = None
                self.is_camera_active = False
    ...
    def _record_video(self):
        """Continuously record 30-second videos with a 5-second interval and manage video file storage."""
        try:
            while self.is_recording:
                ...
                # Add the video file to the queue and delete the oldest if necessary
                self.video_queue.append(video_filename)
                if len(self.video_queue) > 5:
                    oldest_video = self.video_queue.popleft()
    ...
    def start_emotion_analysis(self):
        ...
        if not self.is_emotion_analyzing:
            ...
            self.emotion_evaluator.start_evaluation()
            print("开始情绪分析...")
    ...
    def stop_emotion_analysis(self):
        ...
        if self.is_emotion_analyzing:
            self.is_emotion_analyzing = False
            self.emotion_evaluator.stop_evaluation()
            print("停止情绪分析")
